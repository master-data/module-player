...
- **Demo modules**: `Arkanoid.sid` (`Arkanoid`) by Martin Galway for Imagine (1987); `VESURI - Major Release.mod` (`Major Release`) by Vesuri da Jormas; `GSLINGER.MOD` (`Guitar Slinger`) by Jogeir Liljedahl / Noiseless; `cust.hybris-title` (`Hybris title`, Custom format) by Paul van der Valk for Discovery Software / Cope-Com (1988); `di.partyland` by Olof Gustafsson / Digital Illusions; `elw-lock.xm` (`Dead lock`) by Elwood; `funky stars.xm` (`Hybrid song 2:20`, XM) by Quazar of Sanxion (1996); `Last_Ninja.sid` (`The Last Ninja`) by Ben Daglish and Anthony Lees; `onward.xm` (`Onward`) by Jugi / Complex; `sainahi_circles.mod` (`sainahi circles`, MOD) by A. Mikkonen; and `ghost battle (level 1).hipc` through `ghost battle (level 5).hipc` by Jochen Hippel. See [ACKNOWLEDGMENTS.md](ACKNOWLEDGMENTS.md) for the complete credit record.
