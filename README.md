...
- **Demo modules**: `VESURI - Major Release.mod` (`Major Release`) by Vesuri da Jormas; `GSLINGER.MOD` (`Guitar Slinger`) by Jogeir Liljedahl / Noiseless; `cust.hybris-title` (`Hybris title`, Custom format) by Paul van der Valk for Discovery Software / Cope-Com (1988); `di.partyland` by Olof Gustafsson / Digital Illusions; `elw-lock.xm` (`Dead lock`) by Elwood; `Last_Ninja.sid` (`The Last Ninja`) by Ben Daglish and Anthony Lees; `onward.xm` (`Onward`) by Jugi / Complex; and the five `ghost battle (level N).hipc` modules by Jochen Hippel. See [ACKNOWLEDGMENTS.md](ACKNOWLEDGMENTS.md) for the complete credit record.
