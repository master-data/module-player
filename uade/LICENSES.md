...
| Demo modules | `../demo/assets/music/` | `GSLINGER.MOD` (`Guitar Slinger`) by Jogeir Liljedahl / Noiseless; `VESURI - Major Release.mod` (`Major Release`) by Vesuri da Jormas; `cust.hybris-title` (`Hybris title`, Custom format) by Paul van der Valk for Discovery Software / Cope-Com (1988); `di.partyland` (`Partyland`) by Olof Gustafsson / Digital Illusions; `elw-lock.xm` (`Dead lock`) by Elwood; `Last_Ninja.sid` (`The Last Ninja`) by Ben Daglish and Anthony Lees; `onward.xm` (`Onward`) by Jugi / Complex; and `ghost battle (level 1).hipc` through `ghost battle (level 5).hipc` by Jochen Hippel. Full credits are in `../ACKNOWLEDGMENTS.md`. |
