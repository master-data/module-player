...
| Demo modules | `../demo/assets/music/` | `GSLINGER.MOD` (`02`) by Jogeir Liljedahl / Noiseless; `di.partyland` by Olof Gustafsson / Digital Illusions; `Last_Ninja.sid` (`The Last Ninja`) by Ben Daglish and Anthony Lees; `onward.xm` (`Onward`) by Jugi / Complex; and `ghost battle (level 1).hipc` through `ghost battle (level 5).hipc` by Jochen Hippel. Full credits are in `../ACKNOWLEDGMENTS.md`. |
